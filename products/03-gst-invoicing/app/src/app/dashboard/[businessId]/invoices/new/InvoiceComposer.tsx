 ...
 * the user sees identical to what they get back from /api/...invoices.
 *
 * HSN autocomplete: 200ms-debounced fetch to /api/hsn/search. Pulls
 * common defaults on focus, narrows on type. Picking a code auto-fills the
 * GST rate from the master.
 *
 * Submit: POST /api/businesses/{id}/invoices with the line drafts. The
...
export function InvoiceComposer({ businessId, sellerStateCode, gstScheme, customers }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      startTransition(() => router.push(`/dashboard/${businessId}/invoices/${id}`));
    } catch (err) {
...
function HsnAutocomplete({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/hsn/search?q=${encodeURIComponent(value)}&limit=10`);
        if (res.ok) {
