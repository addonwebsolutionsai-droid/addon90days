...
/**
 * CreateBusinessForm — client island for /dashboard/taxpilot/new.
 *
 * Posts to POST /api/p03/businesses. Auth is via the Clerk session cookie
 * which the API route reads via auth() — no Bearer token needed for same-
 * origin form submits.
 *
 * After success, navigates to /dashboard/taxpilot/<id>.
 */
...
export function CreateBusinessForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/p03/businesses", {
        method:  "POST",
      ...
      startTransition(() => router.push(`/dashboard/taxpilot/${id}`));
    } catch (err) {
