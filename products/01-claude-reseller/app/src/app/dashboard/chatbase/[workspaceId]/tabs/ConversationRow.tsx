...
function ConversationDrawer({ conversation, onClose }: DrawerProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError("");
      try {
        const res = await fetch(`/api/p02/conversations/${conversation.id}/messages`);
  ...
  async function handleTakeOver() {
    setTakingOver(true);
    try {
      const res = await fetch(`/api/p02/conversations/${conversation.id}/take-over`, {
        method: "POST",
      });
  ...
  async function handleSend(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSending(true);
    try {
      const res = await fetch(`/api/p02/conversations/${conversation.id}/send`, {
        method: "POST",
