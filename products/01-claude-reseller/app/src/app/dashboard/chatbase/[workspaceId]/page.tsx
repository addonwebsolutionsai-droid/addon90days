/**
 * /dashboard/chatbase/[workspaceId] — workspace overview with tab nav
 *
 * Component tree:
...
export default async function WorkspacePage({ params, searchParams }: PageProps) {
  ...
  const workspace = await apiFetch<P02Workspace>(
    `/api/p02/workspaces/${workspaceId}`,
    token,
    host
  );
  ...
  const [conversations, kbDocs, intents] = await Promise.all([
    activeTab === "conversations"
      ? apiFetch<P02Conversation[]>(
          `/api/p02/workspaces/${workspaceId}/conversations`,
          token,
          host
        )
    ...
    activeTab === "knowledge"
      ? apiFetch<P02KbDoc[]>(
          `/api/p02/workspaces/${workspaceId}/kb/list`,
          token,
          host
        )
      : Promise.resolve(null),
    activeTab === "intents" || activeTab === "settings"
      ? apiFetch<P02Intent[]>(
          `/api/p02/workspaces/${workspaceId}/intents`,
          token,
          host
        )
      : Promise.resolve(null),
