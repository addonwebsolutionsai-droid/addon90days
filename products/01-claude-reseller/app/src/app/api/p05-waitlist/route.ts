/**
 * POST /api/p05-waitlist
 *
 * Captures email addresses for the ConnectOne (P05) IoT pilot program.
 * Inserts into p05_waitlist table via service-role client (bypasses RLS).
 *
 * Idempotent: duplicate email returns { ok: true, dedupe: true } -- never 409.
 * Validation: Zod email + 200-char max length.
 *
 * NOTE: p05_waitlist is not yet in database.types.ts (migration 008 not applied).
...
/** Untyped Supabase admin client -- used only because p05_waitlist is not yet
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (error) {
    ...
    console.error("[p05-waitlist] insert error:", error.code, error.message);
    return NextResponse.json(
