/**
 * POST /api/p02-waitlist
 *
 * Captures email addresses for the ChatBase (P02) beta waitlist.
 * Inserts into p02_waitlist table via service-role client (bypasses RLS).
 *
 * Idempotent: duplicate email returns { ok: true, dedupe: true } -- never 409.
 * Validation: Zod email + 200-char max length.
 *
 * NOTE: p02_waitlist is not yet in database.types.ts (migration 004 not applied).
...
/** Untyped Supabase admin client -- used only because p02_waitlist is not yet
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (error) {
    ...
    console.error("[p02-waitlist] insert error:", error.code, error.message);
    return NextResponse.json(
