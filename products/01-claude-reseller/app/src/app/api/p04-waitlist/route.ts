/**
 * POST /api/p04-waitlist
 *
 * Captures email addresses for the TableFlow (P04) beta waitlist.
 * Inserts into p04_waitlist table via service-role client (bypasses RLS).
 *
 * Idempotent: duplicate email returns { ok: true, dedupe: true } -- never 409.
 * Validation: Zod email + 200-char max length.
 *
 * NOTE: p04_waitlist is not yet in database.types.ts (migration 007 not applied).
...
/** Untyped Supabase admin client -- used only because p04_waitlist is not yet
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (error) {
    ...
    console.error("[p04-waitlist] insert error:", error.code, error.message);
    return NextResponse.json(
