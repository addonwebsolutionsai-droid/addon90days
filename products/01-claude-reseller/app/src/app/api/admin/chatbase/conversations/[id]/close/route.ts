/**
 * POST /api/admin/chatbase/conversations/{id}/close
 *
 * Marks a conversation closed. Useful for cleaning out the escalated queue.
 *
 * Auth: requireAdmin.
 */
...
export async function POST(_req: Request, ctx: RouteContext): Promise<NextResponse> {
  ...
  await updateConversationStatus(conversationId, "closed", conversation.last_intent ?? undefined);
  return NextResponse.json({ data: { ok: true, status: "closed" } });
}
