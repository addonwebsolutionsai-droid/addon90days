 ...
 *
 * Posts to /api/admin/chatbase/conversations/{id}/{action}. After a
...
export function AdminConversationActions({ conversationId, status }: Props) {
  ...
  async function action(kind: "take-over" | "close") {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/chatbase/conversations/${conversationId}/${kind}`, {
        method: "POST",
      });
      if (!res.ok) {
  ...
  async function send() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/chatbase/conversations/${conversationId}/send`, {
        method:  "POST",
