      ...
      >
        {STATUS_TABS.map((tab) => {
          ...
          return (
            <Link
              key={tab.key}
              href={tab.key === "all" ? "/admin/p03-taxpilot" : `/admin/p03-taxpilot?status=${tab.key}`}
...
function InvoiceRow({ inv }: { inv: InvoiceAdminRow }) {
  return (
    <Link
      href={`/admin/p03-taxpilot/businesses/${inv.business_id}`}
...
function BusinessRow({ biz: b }: { biz: BusinessAdminRow }) {
  return (
    <Link
      href={`/admin/p03-taxpilot/businesses/${b.id}`}
